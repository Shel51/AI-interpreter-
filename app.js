...
async function translate({ q, source, target }) {
  try {
    ...
    console.warn('LibreTranslate failed, fallback to MyMemory', e);
...
function pickKannadaVoice() {
  ...
  return byLang('kn-in') || voices.find(v=>(v.lang||'').toLowerCase().startsWith('kn')) ||
...
btnStart.addEventListener('click', ()=>{
  setError('');
  if (!hasSTT()){ setError('Speech recognition not supported. Use Chrome.'); return; }
  knHeard.value = ''; enMeaning.value=''; myReplyEn.value=''; knReply.value='';
  try { recognition.start(); setListening(true);} 
  catch { setError('Could not start mic. Allow mic permission and retry.'); }
});
...
btnTranslateToEn.addEventListener('click', async ()=>{
  ...
  if (!q){ setError('No Kannada captured. Try again.'); return; }
  ...
  }catch(e){ setError('Translation failed. Please retry.'); }
});

btnTranslateToKn.addEventListener('click', async ()=>{
  setError('');
  const q = myReplyEn.value.trim();
  if (!q){ setError('Type your reply in English first.'); return; }
  ...
  try{
    ...
    try { await speakKannada(kn); } catch(e){ setError('Speech failed. You can still copy the Kannada text.'); }
    setStatus('Done.');
  }catch(e){ setError('Translation failed. Please retry.'); }
});
...
if (hasTTS()){
  speechSynthesis.onvoiceschanged = ()=>{
    ttsStatus.textContent = 'TTS: ' + (speechSynthesis.getVoices().length ? 'Available' : 'No voices found');
  }
}
